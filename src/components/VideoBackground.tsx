...
// ── Fondo de video con scroll + reproductor global ───────────────────────────
...
//     MP4 vía <video> — NO tienen la restricción de embed de YouTube.
//  2) Si no, cae a una lista curada de YouTube (iframe).

import React, { useState, useEffect, useRef, useCallback } from 'react';
import { Volume2, VolumeX, Play, Pause, SkipForward, SkipBack, FastForward, ChevronUp, ChevronDown } from 'lucide-react';
...
const PALETTE = ['34,211,238', '255,45,143', '255,77,77', '250,204,21', '74,222,128', '167,139,250'];

// Lista curada de respaldo (YouTube) hasta que existan MP4 propios.
const CURATED: Scene[] = [
  { id: 'yoasobi', author: 'YOASOBI / Vocaloid', accent: '34,211,238', tracks: [{ title: 'Idol (アイドル)', youtubeId: 'ZRtdQ81jPUQ' }] },
  { id: 'caramell', author: 'Caramella Girls', accent: '255,45,143', tracks: [{ title: 'Caramelldansen', youtubeId: 'A67GrVdEg94' }] },
  { id: 'fnaf', author: 'The Living Tombstone', accent: '255,77,77', tracks: [{ title: 'FNAF 2 Song', youtubeId: 'd1wK9FzN96w' }] },
  { id: 'undertale', author: 'Undertale', accent: '250,204,21', tracks: [{ title: 'Stronger Than You', youtubeId: 'co5Zo6Ng9-c' }] },
  { id: 'minecraft', author: 'Zarcort & Kronno', accent: '74,222,128', tracks: [{ title: 'Creeper vs Zombie', youtubeId: '5m288qNNDw0' }] },
];
...
export default function VideoBackground() {
  ...
  useEffect(() => {
    if (usingMp4 && vidRef.current) {
      if (videoOn) vidRef.current.play().catch(() => {});
      else vidRef.current.pause();
    }
  }, [videoOn, usingMp4, track]);
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 -z-10 overflow-hidden bg-background pointer-events-none">
        ...
        <div className="absolute inset-0" style={{ background: `linear-gradient(180deg, rgba(10,10,15,0.78), rgba(10,10,15,0.9)), radial-gradient(circle at 50% 0%, rgba(${scene.accent},0.10), transparent 55%)` }} />
      </div>

      {/* Audio global de YouTube (iframe oculto sin mute). El MP4 usa ref.muted. */}
      {audioOn && !usingMp4 && track.youtubeId && (
      ...
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40 w-[calc(100%-2rem)] max-w-md">
        <div className="card bg-background/85 backdrop-blur-md p-2.5 flex items-center gap-2" style={{ borderColor: `rgba(${scene.accent},0.35)` }}>
          ...
          <span className="h-2 w-2 rounded-full shrink-0" style={{ background: accent, boxShadow: audioOn ? `0 0 8px ${accent}` : 'none' }} />
          {!collapsed && (
            <div className="min-w-0 flex-1">
              <p className="text-[10px] uppercase tracking-wider text-muted-2 truncate">
                Fondo · {scene.author} {usingDownloads && <span className="text-neon-pink">· propio</span>}
              </p>
